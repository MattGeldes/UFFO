...
import { generatePDFAndSendEmail } from "@/lib/actions/email-actions"
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (!result.isEmailJSConfigured) {
          alert(
            "El PDF se ha generado y descargado correctamente. Para habilitar el envío automático por email, configura EmailJS en las variables de entorno del servidor.",
          )
        } else if (result.emailSent) {
          alert("¡Formulario enviado exitosamente! El PDF se ha descargado y enviado por email.")
        } else {
          alert(`El PDF se ha generado correctamente, pero hubo un problema enviando el email: ${result.emailError}`)
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-lg">Servicio Seleccionado</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            {formData.selectedService === "subscription" ? (
              // Show selected subscription plan instead of budget
              ...
              // Show budget for other services
