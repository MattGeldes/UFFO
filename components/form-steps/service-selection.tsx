          ...
          return (
            <Card
              ...
              className={`cursor-pointer transition-all duration-200 hover:shadow-lg border-2 ${
                service.featured
                  ? "ring-2 ring-primary/20 bg-gradient-to-r from-primary/5 to-primary/10 border-primary/30"
                  : ""
              } ${isSelected ? "border-primary bg-primary/5 shadow-lg" : "border-border hover:border-primary/50"}`}
              onClick={() => onServiceSelect(service.id)}
            >
              <CardContent className="p-6 min-h-[240px] flex flex-col justify-center">
                {service.featured && (
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-foreground mb-2 text-balance text-center">{service.title}</h3>
                    <p className="text-sm text-muted-foreground text-pretty text-center">{service.description}</p>
                  </div>
                  {isSelected && (
